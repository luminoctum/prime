...
void FFTVariable::make() {
	...
	for (int j = 0; j < nyh; j++) {
		...
		plan_forward[j] = fftw_plan_r2r_1d(nxh, value + j * nxh, f_value + j * nxh,
				FFTW_R2HC, FFTW_ESTIMATE);
		//plan_backward[j] = fftw_plan_r2r_1d(nx, f_value + j * nx, value + nh + j * nxh,
		plan_backward[j] = fftw_plan_r2r_1d(nxh, f_value + j * nxh, value + j * nxh,
...
std::string FFTVariable::value_info() const {
    ...
    for (int j = 0; j < nyh; j++) {
        ...
        for (int i = 0; i < nxh; i++) {
            std::stringstream ss;
            //ss << std::setw(8) << f_value[i + j * nx];
            ss << std::setw(8) << f_value[i + j * nxh];
            result += ss.str();
        }
        result += "\n";
    }
